/* Rubik's Cube Solver — Complete Frontend */
...
class App {
  ...
  bindEvents() {
    ...
    document.getElementById('speed-slider').addEventListener('input', (e) => {
      // Speed slider not connected for now
    });
  }
  ...
  async scramble() {
    ...
    try {
      const res = await fetch('http://localhost:8000/api/scramble');
  ...
  async captureFace() {
    ...
    canvas.toBlob(async (blob) => {
      ...
      try {
        const res = await fetch('http://localhost:8000/api/detect/single', {
          method: 'POST',
          body: formData
        });
        const data = await res.json();
